...
function Testimonials() {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: testimonialData, status } = await axiosInstance.get(
      ...
      if (status === 200) {
        ...
        setTotalPages(testimonialData?.totalPages);
        setTotalData(testimonialData?.totalItems);
      } else {
  ...
  return (
    <CardLayout>
      ...
      {/* <Modal isOpen={viewModal} onClose={setViewModal} title={"View Testimonial"}>
        <ViewTestimonial />
      </Modal> */}
      {archiveModal && (
        <ArchiveModal
